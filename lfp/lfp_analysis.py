...
for a in range(16):
    ...
    for j in range(data_in.shape[1]):
        ...
        freq_range = (1, 150)  # Define the frequency range of interest
        ...
        b, c = scipy.signal.butter(3, [1/fNQ, 40/fNQ], 'band')
        ...
        for k in range(n_epochs_t):
            ...
            chan_1_epoched_fft_t[:,k] = (np.abs(fft(epoch_t*window_t)) / np.sum(window_t)) ** 2 ### changed a bit here
